...
int power_supply_set_floated_charger(struct power_supply *psy, int is_float)
{
	...
	if (psy->set_event_property)
		return psy->set_event_property(psy, POWER_SUPPLY_PROP_FLOATED_CHARGER, &ret);

	return -ENXIO;
}
EXPORT_SYMBOL_GPL(power_supply_set_floated_charger);
int power_supply_set_usb_driver_uninstall(struct power_supply *psy, int is_drv_uninstall)
{
	const union power_supply_propval ret = {is_drv_uninstall,};

	if (psy->set_event_property)
		return psy->set_event_property(psy, POWER_SUPPLY_PROP_DRIVER_UNINSTALL, &ret);
